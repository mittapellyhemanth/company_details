...
export default function ProjSalesSearchView(){
    ...
    const decryptedProjectDatay = CryptoJS.AES.decrypt(encryptedProjectData, "employeeSalesSearch").toString(CryptoJS.enc.Utf8);
...
return navigate('/v2/das/sales/pro/view')
  ...
  <button className="cancel-btn" onClick={handleGoBack}>BACK</button>
      ...
      {error ? <p className="error">{error}</p> :   <SalesStatus data={projectStatusData} comesFrom="Sales"/> }
       </div>
    </>
}