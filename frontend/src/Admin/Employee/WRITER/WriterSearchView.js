...
export default function WriterSearchView(){
    ...
    const [projectStatusData, setProjectStatusData] = useState([]);
      ...
      useEffect(() => {
        ...
        const decryptedProjectDatay = CryptoJS.AES.decrypt(encryptedProjectData, "employeewriterSearch").toString(CryptoJS.enc.Utf8);
...
const handleGoBack = ()=>{
return navigate('/v2/writer/project/status')
}
  ...
  <button className="cancel-btn" onClick={handleGoBack}>BACK</button>
      ...
      {error ? <p className="error">{error}</p> :  <WriterStatus data={projectStatusData} /> }
       </div>
    </>
}