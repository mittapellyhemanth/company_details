...
export default function DesignerSearchView(){
    ...
    const [projectStatusData, setProjectStatusData] = useState([]);
      ...
      useEffect(() => {
        ...
        const decryptedProjectDatay = CryptoJS.AES.decrypt(encryptedProjectData, "employeeDesignerSearch").toString(CryptoJS.enc.Utf8);
...
return navigate('/v2/design/project/status')
  ...
  <button className="cancel-btn" onClick={handleGoBack}>BACK</button>
      ...
      {error ? <p className="error">{error}</p> :  <DesignerStatus data={projectStatusData} />}
       </div>
    </>
}